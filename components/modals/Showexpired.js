...
const Showexpired = ({
  setExpiredPopup,
  hideModal,
  totalExpiredAllProjects,
  expiredAllProjectsAll,
}) => {
  ...
  let pageNumber = 1;
  let PageSize = 10;
  const showPages = Math.ceil(totalExpiredAllProjects.length / PageSize);
  const renderTD = () => {
    let td = [];
  ...
  const handlePageChange = (e) => {
    dispatch(getExpiredProjectDataOfUser(e, PageSize, userTokendata.userid));
  };
      ...
      >
        ...
        <Modal.Body>
          <table>
            ...
            <tbody>
              {expiredAllProjectsAll?.length > 0 ? (
                expiredAllProjectsAll.map((item, index) => {
                  return (
                    <tr key={index}>
          ...
          {totalExpiredAllProjects.length > 10 ? (
